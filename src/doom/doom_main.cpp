/* Doom Game run on an Arduino - Code modified by Max Imagination
 *  
 *  Please ensure pinouts and change them if neccessary according to your board
 *  
 *  YouTube: @MaxImagination
 *  Instagram: @max.imagination
 *  
 *  Original code author / source: Unknown
*/
...
// Forward declarations to fix .ino to .cpp conversion
void setup(void);
...
void renderGun(uint8_t gun_pos, double amount_jogging) {
  ...
  char y = RENDER_HEIGHT - gun_pos + fabs(cos((double) millis() * JOGGING_SPEED)) * 8 * amount_jogging;
...
void loopGamePlay() {
  ...
  Serial.println("LEVEL INIT");

  Serial.print("Player X=");
  Serial.println(player.pos.x);

  Serial.print("Player Y=");
  Serial.println(player.pos.y);
